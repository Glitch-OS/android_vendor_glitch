...
ifeq ($(TARGET_VENDOR_SHOW_MAINTENANCE_VERSION),true)
    GLITCH_VERSION_MAINTENANCE := $(PRODUCT_VERSION_MAINTENANCE)
else
    GLITCH_VERSION_MAINTENANCE := 0
endif

# Set CUSTOM_BUILD_TYPE from the env RELEASE_TYPE, for jenkins compat

ifndef CUSTOM_BUILD_TYPE
    ifdef RELEASE_TYPE
        # Starting with "LINEAGE_" is optional
        RELEASE_TYPE := $(shell echo $(RELEASE_TYPE) | sed -e 's|^LINEAGE_||g')
        CUSTOM_BUILD_TYPE := $(RELEASE_TYPE)
    endif
endif

# Filter out random types, so it'll reset to UNOFFICIAL
ifeq ($(filter OFFICIAL NIGHTLY SNAPSHOT EXPERIMENTAL,$(CUSTOM_BUILD_TYPE)),)
    CUSTOM_BUILD_TYPE :=
endif

ifdef CUSTOM_BUILD_TYPE
    ifneq ($(CUSTOM_BUILD_TYPE), SNAPSHOT)
    ...
    # If CUSTOM_BUILD_TYPE is not defined, set to UNOFFICIAL
...
ifeq ($(CUSTOM_BUILD_TYPE), OFFICIAL)
    ifndef TARGET_VENDOR_RELEASE_BUILD_ID
        GLITCH_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(LINEAGE_BUILD)
    else
        ifeq ($(TARGET_BUILD_VARIANT),user)
            ifeq ($(GLITCH_VERSION_MAINTENANCE),0)
                GLITCH_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(LINEAGE_BUILD)
            else
                GLITCH_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(GLITCH_VERSION_MAINTENANCE)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(LINEAGE_BUILD)
            endif
        else
            GLITCH_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(LINEAGE_BUILD)
        endif
    ...
    ifeq ($(GLITCH_VERSION_MAINTENANCE),0)
        ifeq ($(GLITCH_VERSION_APPEND_TIME_OF_DAY),true)
            GLITCH_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d_%H%M%S)-$(CUSTOM_BUILD_TYPE)$(LINEAGE_EXTRAVERSION)-$(LINEAGE_BUILD)
        else
            GLITCH_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)-$(CUSTOM_BUILD_TYPE)$(LINEAGE_EXTRAVERSION)-$(LINEAGE_BUILD)
        endif
    else
        ifeq ($(GLITCH_VERSION_APPEND_TIME_OF_DAY),true)
            GLITCH_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(GLITCH_VERSION_MAINTENANCE)-$(shell date -u +%Y%m%d_%H%M%S)-$(CUSTOM_BUILD_TYPE)$(LINEAGE_EXTRAVERSION)-$(LINEAGE_BUILD)
        else
            GLITCH_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(GLITCH_VERSION_MAINTENANCE)-$(shell date -u +%Y%m%d)-$(CUSTOM_BUILD_TYPE)$(LINEAGE_EXTRAVERSION)-$(LINEAGE_BUILD)
        endif
    endif
endif
...
LINEAGE_DISPLAY_VERSION := $(GLITCH_VERSION)

ifneq ($(PRODUCT_DEFAULT_DEV_CERTIFICATE),)
ifneq ($(PRODUCT_DEFAULT_DEV_CERTIFICATE),build/target/product/security/testkey)
    ifneq ($(CUSTOM_BUILD_TYPE), UNOFFICIAL)
        ifndef TARGET_VENDOR_RELEASE_BUILD_ID
        ...
        ifeq ($(GLITCH_VERSION_MAINTENANCE),0)
            ...
            LINEAGE_DISPLAY_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(GLITCH_VERSION_MAINTENANCE)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(LINEAGE_BUILD)
        endif
    endif
endif
endif
